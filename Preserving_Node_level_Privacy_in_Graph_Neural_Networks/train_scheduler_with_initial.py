...
class BlackBoxAuditor:
    ...
    def predict_and_extract_losses_from_dataset(self, dataset, num_canaries=3000):
        ...
        with torch.no_grad():
            ...
            # 计算 softmax
            softmax_weights = torch.nn.functional.softmax(outputs, dim=1)  # [num_canaries, num_classes]
            canary_softmax_mean = softmax_weights.max(dim=1)[0].mean().item()  # 最大 softmax 值的均值
        ...
        print(f"  - Canary Softmax 均值（最大值）: {canary_softmax_mean:.4f}")
    ...
    def predict_and_extract_losses(self, data_loader):
        ...
        with torch.no_grad():
            for i, batch in enumerate(data_loader):
                ...
                softmax_weights = torch.nn.functional.softmax(outputs, dim=1)
                canary_softmax_list.append(softmax_weights.max(dim=1)[0].cpu().numpy())
        
        canary_softmax_all = np.concatenate(canary_softmax_list)
        canary_softmax_mean = canary_softmax_all.mean()
...
class trainer:
    def __init__(self, *, model, optimizer, loaders, device, criterion, args, enable_audit=False, num_canaries=0, canary_mask=None, dataset=None, pretrain_loader=None, enable_pretrain=False, pretrain_epochs=5):
        self.model = model
        ...
        self.pretrain_loader = pretrain_loader  # 用于非私有预训练的loader（辅助数据集）
        self.enable_pretrain = enable_pretrain  # 是否启用非私有预训练
        self.pretrain_epochs = pretrain_epochs  # 预训练轮次
    ...
    def run_non_private_pretrain(self, pretrain_epochs=5, pretrain_loader=None):
        """
        非私有预训练阶段：使用标准SGD在辅助数据集上训练，以构造最坏情况初始参数
        
        这个阶段的目标是：
        1. 让"正常"样本的梯度缩小
        2. 让目标样本（Canary）的梯度信号最大化
        3. 通过优化初始参数θ₀来最大化信噪比
        
        Args:
            pretrain_epochs: 预训练轮数
            pretrain_loader: 用于预训练的数据加载器（通常是Test集作为辅助数据集）
        """
        if pretrain_loader is None:
            self.write_log("ERROR: pretrain_loader not provided for non-private pretraining")
            return
        
        self.write_log(f"\n{'='*60}")
        self.write_log(f"启动非私有预训练阶段（构造最坏情况初始参数θ₀）")
        self.write_log(f"{'='*60}")
        self.write_log(f"预训练轮次: {pretrain_epochs}")
        self.write_log(f"邻域样本数: {len(pretrain_loader.dataset)}")
        ...
        for epoch in range(pretrain_epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(pretrain_loader, desc=f"Pretrain Epoch {epoch+1}/{pretrain_epochs}")):
                # 处理不同的batch格式
                if len(batch) == 3:
                    x, targets, _ = batch
                else:
            ...
            self.write_log(f"  [Pretrain Epoch {epoch+1}/{pretrain_epochs}] Loss: {avg_epoch_loss:.4f}")
        ...
        self.write_log(f"✓ 非私有预训练完成，耗时: {pretrain_time:.2f}s")
        self.write_log(f"  最优损失值: {best_loss:.4f}")
        self.write_log(f"{'='*60}\n")
        
        # ⚠️  关键：重置优化器，清除预训练的历史状态
        # 防止预训练的优化器状态（如Adam动量）污染正式的DP-GNN训练
        self.write_log(f"[重要] 重置优化器状态，确保正式训练的纯净性")
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.args.lr)
        self.write_log(f"✓ 优化器已重置\n")
        
        # 保存预训练参数
        self.json_recorder.add_record('pretrain_info', {
            'enable_pretrain': True,
            'pretrain_epochs': pretrain_epochs,
            'pretrain_loss': float(best_loss),
            'pretrain_time': float(pretrain_time),
            'dataset_size': len(pretrain_loader.dataset),
            'optimizer_reset': True  # 标记优化器已重置
        })
    ...
    def run(self):
        ...
        # 非私有预训练阶段（如果启用）
        if self.enable_pretrain and self.pretrain_loader is not None:
            self.run_non_private_pretrain(pretrain_epochs=self.pretrain_epochs, pretrain_loader=self.pretrain_loader)
            # ⚠️  此时优化器已在 run_non_private_pretrain 中重置，正式训练将使用干净的优化器状态

        self.write_log(f"\n{'='*60}")
        self.write_log(f"启动正式DP-GNN训练阶段")
        self.write_log(f"{'='*60}\n")
        ...
        if self.enable_audit and self.blackbox_auditor:
            ...
            try:
                ...
                # ⚠️  改用mask来判断成员，而不是硬编码的mid_point
                # 这样可以支持任意的成员/非成员分布
                if self.canary_mask is not None:
                    loss_in = canary_losses[self.canary_mask]
                    loss_out = canary_losses[~self.canary_mask]
                    self.write_log(f"[信息] 使用canary_mask划分: IN={loss_in.shape[0]}, OUT={loss_out.shape[0]}")
                else:
                    # 备选方案：如果没有mask，使用mid_point（为了向后兼容）
                    mid_point = self.num_canaries // 2
                    loss_in = canary_losses[:mid_point]
                    loss_out = canary_losses[mid_point:]
                    self.write_log(f"[警告] 未提供mask，使用默认mid_point划分: IN={mid_point}, OUT={self.num_canaries - mid_point}")
                ...
                # 计算Loss分离度指标（关键诊断）
                loss_separation = (avg_loss_out - avg_loss_in) / (avg_loss_in + 1e-8)  # 相对差异
                loss_separation_abs = avg_loss_out - avg_loss_in  # 绝对差异
                
                # 打印详细的Loss分离度分析
                self.write_log('='*60)
                self.write_log('[详细Loss分离度分析]')
                self.write_log('='*60)
                self.write_log('IN样本Loss分布:')
                ...
                self.write_log('='*60)
                self.write_log('[Loss分离度指标 - 关键！！！]')
                self.write_log('='*60)
                self.write_log(f'绝对差异 (μ_OUT - μ_IN):        {loss_separation_abs:+.4f}')
                self.write_log(f'相对差异 ((μ_OUT - μ_IN) / μ_IN): {loss_separation*100:+.2f}%')
                self.write_log(f'中位数差异 (med_OUT - med_IN):    {median_loss_out - median_loss_in:+.4f}')
                self.write_log(f'Q95分位差异 (Q95_OUT - Q95_IN):   {q95_out - q95_in:+.4f}')
                
                # 关键诊断：if Loss分离度太小，审计会失败
                if abs(loss_separation_abs) < 0.01:
                    self.write_log(f'\n⚠️  WARNING: Loss分离度极小(<0.01)！黑盒审计将失败！')
                    self.write_log(f'   原因分析：')
                    self.write_log(f'   - IN和OUT样本学习曲线相同')
                    self.write_log(f'   - 可能是：(1)预训练增强了泛化能力 (2)DP噪声淹没了信号')
                elif abs(loss_separation_abs) < 0.05:
                    self.write_log(f'\n⚠️  Loss分离度较小(<0.05)，审计效果可能有限')
                else:
                    self.write_log(f'\n✓ Loss分离度足够(>{abs(loss_separation_abs):.4f})，应该能进行有效审计')
                
                self.write_log('='*60)
                # 记录原始损失值
                self.json_recorder.add_record('canary_losses', canary_losses.tolist())
                self.json_recorder.add_record('canary_mask', self.canary_mask.tolist() if self.canary_mask is not None else [])
                
                # 绘制Loss分布曲线
                self._plot_loss_distribution(loss_in, loss_out, avg_loss_in, avg_loss_out)
                ...
                self.json_recorder.add_record('audit_metrics', {
                    ...
                    'loss_separation_abs': float(loss_separation_abs),
                    'loss_separation_relative': float(loss_separation),
                    'std_loss_in': float(std_loss_in),
                    'std_loss_out': float(std_loss_out),
                    'median_loss_in': float(median_loss_in),
                    'median_loss_out': float(median_loss_out),
                })
            except Exception as e:
    ...
    def _plot_loss_distribution(self, loss_in, loss_out, avg_loss_in, avg_loss_out):
        """
        绘制IN/OUT样本的Loss分布曲线 - 改进版，更清晰易读
        """
        try:
            import matplotlib.pyplot as plt
            from scipy.stats import gaussian_kde
            ...
            n_in, bins_in, _ = ax1.hist(loss_in, bins=50, alpha=0.7, color='#1f77b4', edgecolor='black', linewidth=0.5, density=True)
            
            # 添加KDE曲线
            if len(loss_in) > 1:
                kde_in = gaussian_kde(loss_in)
                x_range = np.linspace(loss_in.min(), loss_in.max(), 300)
                ax1.plot(x_range, kde_in(x_range), 'b-', linewidth=3, label='KDE')
            
            # 添加统计线
            ax1.axvline(avg_loss_in, color='red', linestyle='--', linewidth=2.5, label=f'Mean={avg_loss_in:.6f}')
            ax1.axvline(np.median(loss_in), color='green', linestyle='--', linewidth=2.5, label=f'Median={np.median(loss_in):.6f}')
            
            ax1.set_xlabel('Loss Value', fontsize=12, fontweight='bold')
            ax1.set_ylabel('Density', fontsize=12, fontweight='bold')
            ax1.set_title(f'IN Samples Distribution (n={len(loss_in)})', fontsize=13, fontweight='bold')
            ax1.legend(fontsize=11, loc='upper right')
            ax1.grid(True, alpha=0.3)
            
            # ========== 第1行右：OUT样本分布 ==========
            ax2 = fig.add_subplot(gs[0, 1])
            n_out, bins_out, _ = ax2.hist(loss_out, bins=50, alpha=0.7, color='#ff7f0e', edgecolor='black', linewidth=0.5, density=True)
            
            # 添加KDE曲线
            if len(loss_out) > 1:
                kde_out = gaussian_kde(loss_out)
                x_range = np.linspace(loss_out.min(), loss_out.max(), 300)
                ax2.plot(x_range, kde_out(x_range), color='darkorange', linewidth=3, label='KDE')
            
            # 添加统计线
            ax2.axvline(avg_loss_out, color='red', linestyle='--', linewidth=2.5, label=f'Mean={avg_loss_out:.6f}')
            ax2.axvline(np.median(loss_out), color='green', linestyle='--', linewidth=2.5, label=f'Median={np.median(loss_out):.6f}')
            
            ax2.set_xlabel('Loss Value', fontsize=12, fontweight='bold')
            ax2.set_ylabel('Density', fontsize=12, fontweight='bold')
            ax2.set_title(f'OUT Samples Distribution (n={len(loss_out)})', fontsize=13, fontweight='bold')
            ...
            cdf_out = np.arange(1, len(sorted_out) + 1) / len(sorted_out)
            
            ax3.plot(sorted_in, cdf_in, 'b-', linewidth=2.5, label=f'IN CDF (mean={avg_loss_in:.6f})', marker='o', markersize=3, alpha=0.7)
            ax3.plot(sorted_out, cdf_out, color='darkorange', linewidth=2.5, label=f'OUT CDF (mean={avg_loss_out:.6f})', marker='s', markersize=3, alpha=0.7)
            
            ax3.set_xlabel('Loss Value', fontsize=12, fontweight='bold')
            ax3.set_ylabel('Cumulative Probability', fontsize=12, fontweight='bold')
            ax3.set_title('Cumulative Distribution Function (CDF) Comparison', fontsize=13, fontweight='bold')
            ...
            # 箱线图
            bp = ax4.boxplot([loss_in, loss_out], labels=['IN', 'OUT'], patch_artist=True, widths=0.6)
            ...
            ax4.set_title('Box Plot Comparison', fontsize=13, fontweight='bold')
            ...
            # 计算统计数据
            q25_in, q75_in = np.percentile(loss_in, [25, 75])
            q25_out, q75_out = np.percentile(loss_out, [25, 75])
            
            stats_text = f"""
STATISTICS SUMMARY

IN Samples (n={len(loss_in)}):
  Mean:          {avg_loss_in:.8f}
  Median:        {np.median(loss_in):.8f}
  Std Dev:       {np.std(loss_in):.8f}
  Min:           {loss_in.min():.8f}
  Max:           {loss_in.max():.8f}
  Q25:           {q25_in:.8f}
  Q75:           {q75_in:.8f}

OUT Samples (n={len(loss_out)}):
  Mean:          {avg_loss_out:.8f}
  Median:        {np.median(loss_out):.8f}
  Std Dev:       {np.std(loss_out):.8f}
  Min:           {loss_out.min():.8f}
  Max:           {loss_out.max():.8f}
  Q25:           {q25_out:.8f}
  Q75:           {q75_out:.8f}
...
DIFFERENCE METRICS:
  Mean Diff:     {avg_loss_out - avg_loss_in:+.8f}
  Median Diff:   {np.median(loss_out) - np.median(loss_in):+.8f}
  Relative Diff: {(avg_loss_out - avg_loss_in) / (avg_loss_in + 1e-10) * 100:+.2f}%
            """
            ...
            plt.suptitle('Loss Distribution Analysis: IN vs OUT', fontsize=16, fontweight='bold', y=0.995)
            
            # 保存图片
            output_dir = Path(self.log_dir) / 'loss_distribution'
            output_dir.mkdir(parents=True, exist_ok=True)
            save_path = output_dir / 'loss_distribution.png'
            plt.savefig(str(save_path), dpi=150, bbox_inches='tight')
            self.write_log(f"✓ Loss分布曲线已保存: {save_path}")
            
            plt.close()
            
        except Exception as e:
            self.write_log(f"绘制Loss分布曲线出错: {e}")
            import traceback
            traceback.print_exc()
